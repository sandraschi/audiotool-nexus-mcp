...
import { Settings, Cpu, Globe, RefreshCcw, ExternalLink, ShieldCheck, AlertCircle } from "lucide-react";
import { useNexusStore } from "../store";

export function SettingsView() {
  const { llmProvider, setLlmProvider, availableModels, selectedModel, setSelectedModel, refreshModels, isLlmActive } = useNexusStore();
    ...
    <div className="max-w-4xl mx-auto space-y-12">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">Configure local hardware and agentic providers.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <section className="glass-panel p-6 space-y-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="text-sm font-bold text-zinc-200 uppercase tracking-wider">Local LLM Node</h2>
          </div>

          <div className="space-y-4">
            <div>
              <label className="text-[10px] uppercase font-bold text-zinc-500 block mb-3">Active Bridge</label>
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                <label className="text-[10px] uppercase font-bold text-zinc-500 block">Model Selection</label>
                <button 
              ...
              {availableModels.length > 0 ? (
                ...
                <div className="p-4 rounded-lg bg-red-500/5 border border-red-500/10 text-red-400/70 text-xs italic">
                  No models eliciting from {llmProvider === "ollama" ? "localhost:11434" : "localhost:1234"}.
                </div>
              )}
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-zinc-900 flex flex-col gap-2">
            ...
            >
              <ExternalLink size={10} /> Visit {llmProvider === "ollama" ? "Ollama" : "LM Studio"} Documentation
            </a>
          </div>
        </section>
        ...
        <section className="glass-panel p-6 space-y-6 opacity-60 pointer-events-none">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h2 className="text-sm font-bold text-zinc-200 uppercase tracking-wider text-muted">Network Mesh</h2>
          </div>
          
          <div className="p-12 text-center text-zinc-700">
             <p className="text-xs italic underline underline-offset-4 decoration-zinc-800">Advanced peer-to-peer telemetry coming soon.</p>
          </div>
        </section>
      </div>
