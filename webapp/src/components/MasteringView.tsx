...
import { Activity, Eye, Gauge, Layers, RefreshCcw, ShieldCheck, Zap } from "lucide-react";
      ...
      <header className="flex justify-between items-end px-1">
        <div>
          ...
          <p className="text-xs text-zinc-500 uppercase tracking-widest font-bold mt-1">
            Final spectral polish & signal integrity
          </p>
        </div>
            ...
            <div className="bg-zinc-950/50 border-b border-white/[0.02] p-4 flex justify-between items-center">
              <div className="flex items-center gap-4">
                ...
                <div className="flex gap-2">
                  <button className="px-2 py-1 rounded bg-amber-500 text-zinc-950 text-[9px] font-bold uppercase tracking-tighter transition-all">
                    FFT 2048
                  </button>
                  <button className="px-2 py-1 rounded bg-zinc-900 border border-zinc-800 text-zinc-500 text-[9px] font-bold uppercase tracking-tighter hover:text-white transition-all">
                    Smooth
                  </button>
                </div>
              </div>
          ...
          <div className="glass-panel p-6 space-y-8 flex flex-col h-full">
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <RefreshCcw
                  size={14}
                  className="text-zinc-700 hover:text-amber-500 transition-colors cursor-pointer"
                />
              </div>
                ...
                {[
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-2 h-2 rounded-full ${fx.active ? "bg-amber-500" : "bg-zinc-700"}`}
                      />
                      <span
                        className={`text-[12px] font-bold ${fx.active ? "text-zinc-100" : "text-zinc-600"}`}
                      >
                        {fx.name}
                      </span>
                    </div>
                    <button
                      className={`text-[9px] uppercase font-bold transition-all px-2 py-1 rounded
