...
import { Sliders, Volume2, Music, Activity } from "lucide-react";
...
export function MixerView() {
  ...
  const tracks = entities.filter(e => 
    ...
    <div className="space-y-8 h-full flex flex-col">
      <header className="flex justify-between items-end px-1">
        <div>
          ...
          <p className="text-xs text-zinc-500 uppercase tracking-widest font-bold mt-1">High-speed agentic mixing grid</p>
        </div>
        <div className="flex gap-4">
          <div className="text-right">
            <p className="text-[10px] uppercase text-zinc-600 font-bold tracking-widest">Master Peak</p>
            <div className="flex gap-0.5 mt-1">
              {[...Array(12)].map((_, i) => (
                <div key={i} className={`w-1.5 h-3 rounded-full ${i > 9 ? 'bg-red-500/40' : i > 7 ? 'bg-amber-500/40' : 'bg-green-500/20'}`} />
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 glass-panel overflow-x-auto overflow-y-hidden p-6 flex gap-4 scrollbar-thin scrollbar-thumb-zinc-800">
        {tracks.length === 0 ? (
          ...
          tracks.map((track, i) => (
            ...
            >
              {/* Metering */}
              <div className="h-48 bg-zinc-900/50 rounded-xl relative overflow-hidden mb-4 p-1 flex justify-center gap-0.5">
                {[...Array(2)].map((_, j) => (
                  <div key={j} className="flex-1 flex flex-col-reverse gap-0.5">
                    {[...Array(20)].map((_, k) => (
                      <div 
                        key={k} 
                        className={`w-full h-1.5 rounded-sm transition-colors duration-300
                          ${Math.random() > 0.3 + (k/40) ? (k > 17 ? 'bg-red-500' : k > 14 ? 'bg-amber-500' : 'bg-green-500/40') : 'bg-zinc-800'}`} 
                      />
                    ))}
                  </div>
                ))}
                <div className="absolute inset-x-0 top-0 h-1/4 bg-gradient-to-b from-zinc-950/80 to-transparent pointer-events-none" />
              </div>
              ...
              <div className="flex-1 flex flex-col items-center gap-4 py-2">
                ...
                <div className="flex flex-col items-center gap-1">
                  <button className="w-7 h-7 rounded bg-zinc-900 border border-zinc-800 text-[10px] font-bold text-zinc-600 hover:text-red-500 hover:border-red-500/30 transition-all">M</button>
                  <button className="w-7 h-7 rounded bg-zinc-900 border border-zinc-800 text-[10px] font-bold text-zinc-600 hover:text-amber-500 hover:border-amber-500/30 transition-all">S</button>
                </div>
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-zinc-800/50">
                ...
                <div className="mt-2 flex justify-center opacity-30 group-hover:opacity-100 transition-opacity">
                  {track.type.toLowerCase().includes('synth') ? <Music size={12} /> : <Activity size={12} />}
                </div>
              </div>
            </motion.div>
          ))
      ...
      <footer className="glass-panel p-4 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <div>
            <span className="text-[10px] uppercase font-bold text-zinc-600 block mb-1">Total Channels</span>
            <span className="text-zinc-100 font-mono text-sm leading-none">{tracks.length}</span>
          </div>
          <div className="w-px h-8 bg-zinc-800" />
          <div>
            <span className="text-[10px] uppercase font-bold text-zinc-600 block mb-1">Signal Path</span>
            <span className="text-amber-500 font-mono text-sm leading-none font-bold">STEREO_ULTRA</span>
          </div>
        </div>
        <div className="flex gap-2">
           <button className="px-4 py-2 rounded-lg bg-zinc-900 border border-zinc-800 text-xs font-bold text-zinc-400 hover:text-white transition-all">Reset Levels</button>
